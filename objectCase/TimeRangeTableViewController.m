...
#import "TimeRangeTableViewController.h"
...
@interface TimeRangeTableViewController ()
...
@implementation TimeRangeTableViewController
...
- (void)viewDidLoad
{
    ...
    NSMutableDictionary *itemThree = [@{ kTitleKey : @"結束日期",
                                         kDateKey : [NSDate date] } mutableCopy];
...
//    NSLog(@"%@",[dateFormat stringFromDate:targetedDatePicker.date]);
//    cell.detailTextLabel.text = @"yayaya";
}
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    ...
    if([segue.identifier isEqualToString:@"gotoSelectPhoto"]){
        SelectImageCollectionViewController *vc = segue.destinationViewController;
        UITableViewCell *cell = [self.tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:1 inSection:0]];
//        NSLog(@"DATE : %@",cell.detailTextLabel.text);
        vc.startDate = cell.detailTextLabel.text;
        
        cell = [self.tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:2 inSection:0]];
        
        if (cell.detailTextLabel.text != nil) {
//            NSLog(@"DATE1 : %@",cell.detailTextLabel.text);
            vc.endDate = cell.detailTextLabel.text;
        } else {
            cell = [self.tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:3 inSection:0]];
//            NSLog(@"DATE2 : %@",cell.detailTextLabel.text);
            vc.endDate = cell.detailTextLabel.text;
        }
